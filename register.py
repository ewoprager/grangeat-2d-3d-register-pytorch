...
def main(*, path: str | None, cache_directory: str, load_cached: bool, regenerate_drr: bool, save_to_cache: bool,
    ...
    zncc, resampled = objective_function.evaluate(
        fixed_image, sinogram3d, transformation=tr.to(device=device), scene_geometry=scene_geometry,
        fixed_image_grid=sinogram2d_grid, plot=colour_limits)
    logger.info(
        "Evaluation: -ZNCC = -{:.4e}".format(
            zncc.item()  # evaluate_direct(fixed_image, vol_data,
            # transformation=tr,
    ...
    if False:
        n = 80
        angle0s = torch.linspace(
            transformation_ground_truth.rotation[0] - torch.pi, transformation_ground_truth.rotation[0] + torch.pi, n)
        angle1s = torch.linspace(
            transformation_ground_truth.rotation[1] - torch.pi, transformation_ground_truth.rotation[1] + torch.pi, n)
        nznccs = torch.zeros((n, n))
        for i in tqdm(range(nznccs.numel())):
            i0 = i % n
            i1 = i // n
            nznccs[i1, i0] = -objective_function.evaluate(
                fixed_image, sinogram3d, transformation=Transformation(
        ...
        if False:
            ...
            for i in tqdm(range(nznccs.numel())):
                ...
                nznccs[i1, i0] = -objective_function.evaluate(
                    fixed_image, sinogram3d, transformation=Transformation(
                        torch.tensor(
                            [angle0s[i0], angle1s[i1], transformation_ground_truth.rotation[2]], device=device),
                        transformation_ground_truth.translation), scene_geometry=scene_geometry,
    ...
    if False:
        ...
        for i in tqdm(range(n)):
            ...
            nznccs[i] = -objective_function.evaluate(
                fixed_image, sinogram3d, transformation=transformation, scene_geometry=scene_geometry,
                fixed_image_grid=sinogram2d_grid)[0]
    ...
    if True:
        def objective(params: torch.Tensor) -> torch.Tensor:
            return -objective_function.evaluate(
                fixed_image, sinogram3d, transformation=Transformation(params[0:3], params[3:6]).to(
                    device=device), scene_geometry=scene_geometry, fixed_image_grid=sinogram2d_grid)[0]

        logger.info("Optimising...")
        param_history = []
        value_history = []
        transformation_start = Transformation(
            -.75 * torch.pi * torch.tensor([1., -1., -1]), torch.tensor([0., 0., 80.]))
        start_params: torch.Tensor = transformation_start.vectorised()

        initial_image = geometry.generate_drr_python(
            vol_data, transformation=transformation_start.to(device=device), voxel_spacing=voxel_spacing,
            detector_spacing=detector_spacing, scene_geometry=scene_geometry, output_size=drr_image.size(),
            samples_per_ray=512)
        _, axes = plt.subplots()
        ...
        if False:  # Using a PyTorch optimiser
            ...
            res = scipy.optimize.basinhopping(
                objective_scipy, start_params.numpy(), T=5.0, minimizer_kwargs={"method": 'Nelder-Mead'})
            toc = time.time()
            logger.info("Done. Took {:.3f}s.".format(toc - tic))
            logger.info(res)
        ...
        objective_function.evaluate(
            fixed_image, sinogram3d, transformation=Transformation(converged_params[0:3], converged_params[3:6]).to(
                device=device), scene_geometry=scene_geometry, fixed_image_grid=sinogram2d_grid, plot=colour_limits)

        final_image = geometry.generate_drr_python(
            vol_data, transformation=Transformation(
                torch.tensor(converged_params[0:3], device=device), torch.tensor(converged_params[3:6], device=device)),
            voxel_spacing=voxel_spacing, detector_spacing=detector_spacing, scene_geometry=scene_geometry,
            output_size=drr_image.size(), samples_per_ray=512)
        _, axes = plt.subplots()
