...
def main(  #
    ...
    if fill_gaps is None:
        ...
        config = ExperimentConfig({  #
            ...
            # ----- preprocessing
            "downsample_level": Constant(1),  # Cartesian([1, 2]),  #
            "truncation_percent": Constant(85),  # Cartesian([80, 90]),  #
            # ----- cropping
            ...
            "apply_weighting": Constant(False),
            # Cartesian([False, True]),  # Cartesian([0.0, 0.25, 0.5, 1.0, 2.0]),  #
            "weight_alpha": Constant(0.5),  # Cartesian([0.0, 0.25, 0.5, 1.0, 2.0]),  #
            "iterations_per_weight_update": Constant(1000),  #
            "sim_metric": Constant("gradient_difference"),  # Constant("gradient_correlation"),
            # ----- registration
            "starting_distance": Constant(5.0),  # Cartesian([5.0, 7.5, 10.0, 12.5, 15.0]),  # Constant(5.0)
            "sample_count_per_distance": Constant(10),  #
            # ----- PSO config
            "particle_count": Constant(2000),  #
            "particle_initialisation_spread": Constant(2.5),  # Constant(5.0),# # Constant(2.5)
