...
The `data_manager()` is a singleton instance of `DAG` used to keep track of data and which data depends on which other
data. Here is an example to illustrate its purpose:


### Motivation behind the `data_manager()`
...
masking, downsampling, projecting etc.... The `DAG` allows you to only define the dependencies between the images, and
will automatically perform the operations that evaluate the images when the dependencies change.
...
To partially solve the former issue, and completely solve the latter, we use the `DAG` as follows:
```Python
from typing import Any
import torch
from reg23_experiments.program.data_manager_singleton import init_data_manager, data_manager, dag_updater

init_data_manager()

data_manager().set_data_multiple(ct_path="/my_ct_volume.nrrd", downsample_ratio=3)

@dag_updater(names_returned=["ct_volume"])
def load_ct_volume(ct_path: str) -> dict[str, Any]:
    return {"ct_volume": load_from_file(ct_path)}

@dag_updater(names_returned=["ct_volume_downsampled"])
...
...
    ...
    ...
    data_manager().set_data("downsample_ratio", 3)
    use_ct_volume_again(data_manager().get("ct_volume_downsampled"))
```
A walk through the above code:
- We first call `init_data_manager()` as good practice, as if we want to pass arguments to the constructor in future,
this is where we'd do so. As we aren't, this isn't actually necessary, as the singleton instance would be constructed
with no arguments on the first call to `data_manager()` if we didn't call `init_data_manager()` first.
- We set some values in the data manager using `data_manager().set_data_multiple(...)`. This just interprets each
keyword argument as a new (name, value) pair to insert into the graph.
- We define two instances of the `Updater` class using the `@dag_updater` decorator, which transforms the decorated
function into an instance of `Updater`. This is an object which is used to define dependencies between nodes in the data
manager graph: the arguments of the function define the variables that are depended upon, and the (key, value) pairs in
the returned dictionary define the variables that are dependent on them. The function itself defines the mapping from 
the 'depended' to the 'dependent'.
