...
class ElectrodesGUI:
    def __init__(self, app_state: AppState):
        ...
        data = self._save_manager.get(self._app_state.dadg.get("xray_path"))
        if data is None:
            self._points_layer = viewer().add_points(ndim=2, size=4.0, name="CI electrodes")
        else:
            self._points_layer = viewer().add_points(data.numpy(), size=4.0, name="CI electrodes")
        self._app_state.dadg.set("electrode_points", data)
    ...
    def _on_layer_change(self, event):
        if event.type == "data":
            ...
            self._save_manager.set(self._app_state.dadg.get("xray_path"), tensor)
