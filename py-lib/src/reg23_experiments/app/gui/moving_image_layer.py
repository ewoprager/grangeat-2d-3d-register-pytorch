...
class _MovingImageLayerManager:
    def __init__(self, *, ctx: AppContext, layer: napari.layers.Layer, dadg_key: str):
        ...
        self._ctx.dadg.observe(dadg_key, "moving_image_manager", self._observer_callback)
    ...
    def _mouse_drag(self, layer, event):
        if event.button == 1 and self._key_states["Ctrl"]:  # Ctrl-left click drag
            ...
            rotation_start = scipy.spatial.transform.Rotation.from_rotvec(
                rotvec=self._ctx.dadg.get("current_transformation").rotation.cpu().numpy())
            yield
            # on move
            while event.type == "mouse_move":
                ...
                prev = self._ctx.dadg.get("current_transformation")
                self._ctx.dadg.set(  #
            ...
            translation_start = self._ctx.dadg.get("current_transformation").translation[0:2].cpu()
            yield
            # on move
            while event.type == "mouse_move":
                dragged = True
...
def add_moving_image_layer(*, ctx: AppContext, namespace: str | None = None) -> napari.layers.Layer:
    ...
    layer.my_plugin = _MovingImageLayerManager(ctx=ctx, layer=layer, dadg_key=moving_image_key)
    return layer
