...
class OptimisationJob:
    def __init__(self, max_iterations: int, op_algo_cls: type, **op_algo_kwargs):
        self._op_algo_cls = op_algo_cls
        self._max_iterations: int = max_iterations
        self._init_state: dict[str, Any] = op_algo_kwargs
        self._op_algo: OptimisationAlgorithm = None
        self._it_count: int = None
        self.reset()
    ...
    def reset(self) -> None:
        self._op_algo: OptimisationAlgorithm = self._op_algo_cls(**self._init_state)
...
class OptimisationWorker(QObject):
    ...
    def run(self):
        logger.info("Optimisation worker started.")
...
def new_op_job(*, parameters: Parameters,
               objective_function: Callable[[torch.Tensor], torch.Tensor]) -> OptimisationJob:
    if parameters.optimisation_algorithm == "pso":
        ...
        return OptimisationJob(  #
            ...
            PsoAlgorithm,  #
            particle_count=500,  #
            starting_pos=mapping_transformation_to_parameters(data_manager().get("current_transformation")),  #
            starting_spread=1.0,  #
            objective_function=objective_function,  #
            device=data_manager().get("device"),  #
        )
    else:
...
class RegisterGUI(widgets.Container):
    def __init__(self, *, parameters: Parameters, objective_function: Callable[[torch.Tensor], torch.Tensor]):
    ...
    def _on_eval_once(self, *args) -> None:
        ...
        self._eval_result_label.value = "{:.4f}".format(
            self._objective_function(mapping_transformation_to_parameters(tr)).item())
    ...
    def _update_op_algo_state_display(self) -> None:
        ...
        self._op_algo_state_display.value = (
            ...
            "}").format(  #
            self._op_job_cache[self._parameters.optimisation_algorithm].iteration_count,  #
            self._op_job_cache[self._parameters.optimisation_algorithm].max_iterations,  #
            self._op_job_cache[self._parameters.optimisation_algorithm].name,  #
            self._op_job_cache[self._parameters.optimisation_algorithm].best,  #
            self._op_job_cache[self._parameters.optimisation_algorithm].best_position,  #
        )
    ...
    def _on_reset_state(self, *args) -> None:
        ...
        self._op_job_cache[  #
            ...
            parameters=self._parameters, objective_function=self._objective_function)
        self._update_op_algo_state_display()
