...
if torch.cuda.is_available():
    ...
    def _weighted_ncc(xs: torch.Tensor, ys: torch.Tensor, weights: torch.Tensor, *,
    ...
    def test_objective_function():
        ...
        weight_alpha = 0.0
        res_of = objective_function(input_.cuda(), fixed_image.cuda(), voxel_spacing.cuda(), inv_hs.cuda(),
                                    source_distance, torch.zeros(2, dtype=torch.float64), detector_spacing.cuda(),
                                    weight_alpha).cpu()

        drrs = project_drrs_batched(input_.cuda(), voxel_spacing.cuda(), inv_hs.cuda(), source_distance,
                                    output_size[-1], output_size[-2], torch.zeros(2, dtype=torch.float64),
                                    detector_spacing.cuda())
        masks = project_drr_cuboid_masks_batched(torch.tensor(input_.size()).cuda(), voxel_spacing.cuda(),
                                                 inv_hs.cuda(), source_distance, output_size[-1], output_size[-2],
        ...
        res_og = _weighted_ncc(drrs, masked_fixeds, weights, dim=(-1, -2)).cpu()

        # print("New: ", res_of)  # print("Old: ", res_og)
