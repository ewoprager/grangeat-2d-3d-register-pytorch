...
#include <reg23_core/ResampleSinogram3D.h>
#include <reg23_core/SinogramClassic3D.h>
#include <reg23_core/SinogramHEALPix.h>
#include <reg23_core/Texture3DCPU.h>
...
__global__ void Kernel_ResampleSinogram3D_CUDA(sinogram_t inputSinogram, const ConstantGeometry geometry,
...
__host__ at::Tensor ResampleSinogram3D_CUDA(const at::Tensor &sinogram3d, const std::string &sinogramType,
	...
	CommonData common =
		ResampleSinogram3D::Common(sinogramType, projectionMatrix, phiValues, rValues, at::DeviceType::CUDA, out);
...
__host__ at::Tensor ResampleSinogram3DCUDATexture(int64_t sinogram3dTextureHandle, int64_t sinogramWidth,
	...
	switch (common.sinogramType) {
	case ResampleSinogram3D::SinogramType::CLASSIC: {
		SinogramClassic3D<Texture3DCUDA> sinogram =
			SinogramClassic3D<Texture3DCUDA>::FromCUDAHandle(sinogram3dTextureHandle, sinogramSize, rSpacing);
		int minGridSize, blockSize;
		cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize,
										   &Kernel_ResampleSinogram3D_CUDA<SinogramClassic3D<Texture3DCPU>>, 0, 0);
		const int gridSize = (static_cast<int>(common.flatOutput.numel()) + blockSize - 1) / blockSize;
	...
	case ResampleSinogram3D::SinogramType::HEALPIX: {
		SinogramHEALPix<Texture3DCUDA> sinogram =
			SinogramHEALPix<Texture3DCUDA>::FromCUDAHandle(sinogram3dTextureHandle, sinogramSize, rSpacing);
		int minGridSize, blockSize;
		cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize,
