...
#include <reg23_core/Texture.h>

namespace reg23 {

unsigned long long CUDATexture2D::Handle() const { return static_cast<unsigned long long>(textureHandle); }

unsigned long long CUDATexture3D::Handle() const { return static_cast<unsigned long long>(textureHandle); }
...
CUDATexture3D::CUDATexture3D(const at::Tensor &tensor, const std::string &addressModeX, const std::string &addressModeY,
	...
	: CUDATexture3D(tensor, StringsToAddressModes<3>({{addressModeX, addressModeY, addressModeZ}})) {}
