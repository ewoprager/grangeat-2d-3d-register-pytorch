...
#include <reg23_core/ProjectDRR.h>
...
__host__ at::Tensor ProjectDRR_CUDA(const at::Tensor &volume, const at::Tensor &voxelSpacing,
	...
	const CommonData common =
		ProjectDRR<Texture3DCUDA>::Common(volume, voxelSpacing, homographyMatrixInverse, sourceDistance, outputOffset,
	...
	Kernel_ProjectDRR_CUDA<<<gridSize, blockSize>>>(
		common.inputTexture, sourceDistance, common.lambdaStart, common.stepSize, common.samplesPerRay,
		common.homographyMatrixInverse, common.inputTexture.MappingWorldToTexCoord(), common.detectorSpacing,
...
__global__ void Kernel_ProjectDRRsBatched_CUDA(Texture3DCUDA volume, double sourceDistance, double stepSize,
											   double volumeDiagLength, int64_t samplesPerRay, int64_t batchCount,
											   double *invHMatrices,
											   Linear<Texture3DCUDA::VectorType> mappingWorldToTexCoord,
											   Vec<double, 2> detectorSpacing, Vec<int64_t, 2> outputSize,
											   Vec<double, 2> outputOffset, float *arrayOut) {
...
__host__ at::Tensor ProjectDRRsBatched_CUDA(const at::Tensor &volume, const at::Tensor &voxelSpacing,
	...
	const Texture3DCUDA::VectorType volumeDiagonal =
		inputSize.StaticCast<Texture3DCUDA::FloatType>() * inputTexture.Spacing();
	...
	at::Tensor flatOutput =
		torch::zeros(at::IntArrayRef({batchCount * outputWidth * outputHeight}), volume.contiguous().options());
	float *resultFlatPtr = flatOutput.data_ptr<float>();
...
__host__ at::Tensor ProjectDRR_backward_CUDA(const at::Tensor &volume, const at::Tensor &voxelSpacing,
	...
	Kernel_ProjectDRR_backward_CUDA<<<gridSize, blockSize, bufferSize>>>(
		std::move(common.inputTexture), sourceDistance, common.lambdaStart, common.stepSize, common.samplesPerRay,
		common.homographyMatrixInverse, common.inputTexture.MappingWorldToTexCoord(), common.detectorSpacing,
		Vec<int64_t, 2>{outputWidth, outputHeight}, common.outputOffset, dLossDDRRFlatPtr, blockSumsPtr);
