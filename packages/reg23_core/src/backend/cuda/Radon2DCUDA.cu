...
#include <reg23_core/Radon2D.h>
#include <reg23_core/Texture2DCUDA.h>
...
__global__ void Kernel_Radon2D_CUDA(Texture2DCUDA textureIn, int64_t numelOut, float *arrayOut,
	...
	arrayOut[threadIndex] =
		scaleFactor * Radon2D<Texture2DCUDA>::IntegrateLooped(textureIn, mappingIndexToTexCoord, samplesPerLine);
}
...
__host__ at::Tensor Radon2D_CUDA(const at::Tensor &image, const at::Tensor &imageSpacing, const at::Tensor &phiValues,
	...
	CommonData common =
		Radon2D<Texture2DCUDA>::Common(image, imageSpacing, phiValues, rValues, samplesPerLine, at::DeviceType::CUDA);
...
__global__ void Kernel_DRadon2DDR_CUDA(Texture2DCUDA textureIn, int64_t numelOut, float *arrayOut,
	...
	const Linear<Vec<double, 2>> mappingIndexToTexCoord = Radon2D<Texture2DCUDA>::GetMappingIndexToTexCoord(
		textureIn, phiValues[threadIndex], rValues[threadIndex], mappingIndexToOffset);
	const Vec<double, 2> dTexCoordDR =
		Radon2D<Texture2DCUDA>::GetDTexCoordDR(textureIn, phiValues[threadIndex], rValues[threadIndex]);
...
int blockSizeToDynamicSMemSize_Radon2D_CUDA_V2(int blockSize) { return blockSize * sizeof(float); }
