...
__global__ void Kernel_Radon3D_CUDA(Texture3DCUDA textureIn, int64_t numelOut, float *arrayOut,
	...
	arrayOut[threadIndex] =
		scaleFactor * Radon3D<Texture3DCUDA>::IntegrateLooped(textureIn, mappingIndexToTexCoord, samplesPerDirection);
}
...
__global__ void Kernel_DRadon3DDR_CUDA(Texture3DCUDA textureIn, int64_t numelOut, float *arrayOut,
	...
	const Linear2<Vec<double, 3>> mappingIndexToTexCoord =
		Radon3D<Texture3DCUDA>::GetMappingIndexToTexCoord(textureIn, phi, theta, r, mappingIToOffset);
...
__global__ void Kernel_Radon3D_CUDA_V3(Linear2<Vec<double, 3>> mappingIndexToTexCoord) {
	...
	buffer[threadIdx.y * blockDim.x + threadIdx.x] =
		tex3D<float>(radon3DV2Consts.textureHandle, texCoord.X(), texCoord.Y(), texCoord.Z());
	...
	for (int64_t cutoff = blockDim.x / 2; cutoff > 0; cutoff /= 2) {
		if (threadIdx.x < cutoff && threadIdx.y < cutoff) {
			buffer[threadIdx.y * blockDim.x + threadIdx.x] +=
				buffer[threadIdx.y * blockDim.x + threadIdx.x + cutoff] +
				buffer[(threadIdx.y + cutoff) * blockDim.x + threadIdx.x] +
				buffer[(threadIdx.y + cutoff) * blockDim.x + threadIdx.x + cutoff];
		}
...
int blockSizeToDynamicSMemSize_Radon3D_CUDA_V2(int blockSize) { return blockSize * sizeof(float); }
...
int blockSizeToDynamicSMemSize_DRadon3DDR_CUDA_V2(int blockSize) { return blockSize * sizeof(float); }
...
__host__ at::Tensor DRadon3DDR_CUDA_V2(const at::Tensor &volume, const at::Tensor &volumeSpacing,
	...
	DRadon3DDRV2Consts constants{common.inputTexture.GetHandle(),
								 samplesPerDirection,
								 common.scaleFactor,
								 patchSumsPtr,
								 volume.sizes()[2],
