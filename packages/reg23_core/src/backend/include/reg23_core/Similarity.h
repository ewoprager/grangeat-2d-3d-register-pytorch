 ...
 * @return  @return a tuple: (ZNCC; sum of values in `a`; sum of values in `b`; the numerator of the fraction shown
 * below; the left sqrt in the denominator of the fraction shown below; the right sqrt in the denominator of the
 * fraction shown below)
 *
 * ZNCC = (N * sum(a_i * b_i) - sum(a_i) * sum(b_i)) / (sqrt(N * sum(a_i^2) - sum(a_i)^2) * sqrt(N * sum(b_i^2) -
 * sum(b_i)^2))
 */
std::tuple<at::Tensor, double, double, double, double, double> NormalisedCrossCorrelation_CPU(const at::Tensor &a,
																							  const at::Tensor &b);
 ...
 */
__host__ std::tuple<at::Tensor, double, double, double, double, double>
NormalisedCrossCorrelation_CUDA(const at::Tensor &a, const at::Tensor &b);
