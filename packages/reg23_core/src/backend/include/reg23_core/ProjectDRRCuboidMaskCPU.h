 ...
 * @brief Calculate the distances of the intersections between the DRR-generation rays and the domain of the volume
 * data.
 * @param volumeSize a tensor of size (3,): the sizes of the volume data tensor in the order (X, Y, Z) - this is the
 * reverse order of tensor.size()
 * @param voxelSpacing a tensor of size (3,): the spacing in the (X, Y, Z) directions between adjactent voxels in the
 * volume
 ...
 * @param faceOutUnitNormals An array of unit vectors, one for each polyhedron face, where each vector is the
 * outward-facing normal vector of its corresponding face
...
__host__ __device__ T RayConvexPolyhedronDistance(const std::array<Vec<T, 3>, faceCount> &facePoints,
	...
	// // polyhedron. If it lies 'outside' of any,	it doesn't intersect the polyhedron. Technically, could just check
	// the
	// // entry point against only the faces adjacent to the intersected face, but that would require expensive analysis
	// of
 ...
 * `ProjectDRRCuboidMask_...` functions
 */
struct ProjectDRRCuboidMask {
	...
	__host__ static CommonData Common(const at::Tensor &volumeSize, const at::Tensor &voxelSpacing,
		...
		for (Vec<float, 3> &v : ret.cuboidAbove.facePoints)
			v.Z() += zSum;
		for (Vec<float, 3> &v : ret.cuboidBelow.facePoints)
			v.Z() -= zSum;
		...
		ret.sourcePositionTransformed =
			MatMul(ret.homographyMatrixInverse, Vec<float, 4>{0.0f, 0.0f, static_cast<float>(sourceDistance), 1.0f})
				.XYZ();
		ret.outputOffset = Vec<float, 2>::FromTensor(outputOffset);
		ret.detectorSpacing = Vec<float, 2>::FromTensor(detectorSpacing);
		ret.flatOutput =
			torch::zeros(at::IntArrayRef({outputWidth * outputHeight}), at::TensorOptions{at::Device{device}});
		return ret;
	}
};
