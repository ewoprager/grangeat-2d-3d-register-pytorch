 ...
 * @param grid a tensor of size (..., 3) containing `torch.float32`s: The grid to sample at; the last dimension
 * represents 3D locations within the `input` volume between (-1, -1, -1) and (1, 1, 1)
 ...
 * @return a tensor of size (...) containing `torch.float32`s: The sampled values in the given volume at the locations
 * in the given grid; this will be the same size as `grid`, minus the last dimension.
 *
 * # Address modes
...
template <typename texture_t> struct GridSample3D {
	...
	__host__ static CommonData Common(const at::Tensor &input, const at::Tensor &grid, const std::string &addressModeX,
		...
		if (out) {
			// out should be a tensor of floats matching all but the last dimension of grid in size, on the chosen
			// device
		...
		ret.flatOutput = out ? out.value().view({-1})
