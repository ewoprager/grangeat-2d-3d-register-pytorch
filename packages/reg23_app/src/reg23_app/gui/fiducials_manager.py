...
class FiducialsManager:
    ...
    def _button_fiducial_register(self, change) -> None:
        ...
        if isinstance(image_2d_full_spacing, Error):
            logger.error(
                f"Could not find 'image_2d_full_spacing' for X-ray '{self._state.register_fiducial_xray_choice}': "
        ...
        if False:
            ...
            image_2d_full_spacing = image_2d_full_spacing.to(device=torch.device("cpu"))
            ...
            targets_2d = xray_point_vectors - 0.5 * image_2d_full_spacing * torch.tensor(image_2d_full.size()).flip(
                dims=(0,))
        ...
        def residuals(pose: np.ndarray) -> np.ndarray:
            self._dadg.set("current_transformation", mapping_parameters_to_transformation(torch.tensor(pose)))
            res = self._dadg.get(dadg_key_prefix + "projected_fiducials")
            if isinstance(res, Error):
                raise Exception(
                    f"Couldn't get projected fiducials for X-ray '{self._state.register_fiducial_xray_choice}'")
            projected_names, projected_points = res
            res = self._dadg.get(dadg_key_prefix + "fiducial_points")
            if isinstance(res, Error):
                raise Exception(f"Couldn't get fiducial points for X-ray '{self._state.register_fiducial_xray_choice}'")
            xray_names, xray_points = res
            if names_not_in_both := set(projected_names) ^ set(xray_names):
                raise Exception(
                    f"The following points did not have counterparts in the other image:\n{list(names_not_in_both)}")
            name_to_index_map = {name: index for index, name in enumerate(xray_names)}
            permutation = torch.tensor([name_to_index_map[name] for name in projected_names])
            projected_points = projected_points[permutation.argsort()]
            return (xray_points - projected_points).flatten().numpy()
