...
class SinogramClassic(Sinogram):
    ...
    def __init__(self, data: torch.Tensor, r_range: LinearRange, *, pad: bool = True):
        ...
        if self.device.type == "cuda":
            self._texture = reg23_core.CUDATexture3D(self.data, "zero", "zero", "zero")
    ...
    def resample(self, ph_matrix: torch.Tensor, fixed_image_grid: Sinogram2dGrid, *,
        ...
        return reg23_core.resample_sinogram3d(self.data, "classic", self.r_range.get_spacing(self.data.size()[2]),
    ...
    def resample_cuda_texture(self, ph_matrix: torch.Tensor, fixed_image_grid: Sinogram2dGrid, *,
        ...
        return reg23_core.resample_sinogram3d_cuda_texture(self._texture, "classic",
    ...
    def resample_python(self, ph_matrix: torch.Tensor, fixed_image_grid: Sinogram2dGrid, *, smooth: float | None = None,
        ...
        ret = reg23_core.grid_sample3d(self.data, grid, "zero", "zero", "zero", out=out)
...
"""
    def grid_sample_smoothed(self, grid: Sinogram3dGrid, *, i_mapping: LinearMapping, j_mapping: LinearMapping,
        ...
        samples = reg23_core.grid_sample3d(self.data, grid, address_mode="wrap")
...
class SinogramHEALPix(Sinogram):
    ...
    def sample(self, grid: Sinogram3dGrid, out: torch.Tensor | None = None) -> torch.Tensor:
        ...
        return reg23_core.grid_sample3d(self.data, grid, "zero", "zero", "zero", out=out)

    def resample(self, ph_matrix: torch.Tensor, fixed_image_grid: Sinogram2dGrid, *,
                 out: torch.Tensor | None = None) -> torch.Tensor:
        return reg23_core.resample_sinogram3d(self.data, "healpix", self.r_range.get_spacing(self.data.size()[0]),
                                              ph_matrix, fixed_image_grid.phi, fixed_image_grid.r, out=out)

    def resample_cuda_texture(self, ph_matrix: torch.Tensor, fixed_image_grid: Sinogram2dGrid, *,
                              out: torch.Tensor | None = None) -> torch.Tensor:
        assert self.device.type == "cuda"
        return reg23_core.resample_sinogram3d_cuda_texture(self._texture, "healpix",
