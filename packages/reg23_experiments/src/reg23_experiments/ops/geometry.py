...
@jaxtyped(typechecker=typechecker)
def ray_cuboid_distance(  #
        cuboid_centre: Float64[torch.Tensor, "3"],  #
        cuboid_half_sizes: Float64[torch.Tensor, "3"],  #
        ray_points: Float64[torch.Tensor, "... 3"],  #
        ray_unit_directions: Float64[torch.Tensor, "... 3"]  #
) -> torch.Tensor:
    """
    ...
    """
    device = cuboid_centre.device
    assert cuboid_half_sizes.device == device
    assert ray_points.device == device
    assert ray_unit_directions.device == device
    ...
    plane_normals = torch.nn.functional.normalize(torch.eye(3, **tensor_kwargs).repeat(2, 1) * deltas)  # size = (6, 3)
    ...
    lambdas_masked = torch.where(dots < -epsilon, lambdas,
                                 torch.tensor(float("-inf"), **tensor_kwargs))  # size = (..., 6)
    ...
    check_indices = torch.remainder(
        entry_indices.unsqueeze(-1) + torch.tensor([1, 2, 4, 5], dtype=torch.int64, device=device),
        6)  # size = (..., 4)
    ...
    lambdas_masked = torch.where(dots > epsilon, lambdas, torch.tensor(float("inf"), **tensor_kwargs))  # size = (.., 6)
    ...
    return torch.where(rays_hit, (exit_lambdas - entry_lambdas).abs(),
                       torch.tensor(0.0, **tensor_kwargs))  # size = (...)
...
def generate_drr(volume_data: torch.Tensor, *, transformation: Transformation, voxel_spacing: torch.Tensor,
    ...
    return reg23_core.project_drrs_batched(volume_data, voxel_spacing, h_matrix_inv.unsqueeze(0),
                                           scene_geometry.source_distance, img_width, img_height,
                                           scene_geometry.fixed_image_offset.to(device=device), detector_spacing)[0]
...
def generate_drr_cuboid_mask(volume_data: torch.Tensor, *, transformation: Transformation, voxel_spacing: torch.Tensor,
    ...
    return reg23_core.project_drr_cuboid_mask(
...
@jaxtyped(typechecker=typechecker)
def generate_drr_python(  #
        volume_data: Float32[torch.Tensor, "p q r"],  #
        *,  #
        transformation: Transformation,  #
        voxel_spacing: Float64[torch.Tensor, "3"],  #
        detector_spacing: Float64[torch.Tensor, "2"],  #
        scene_geometry: SceneGeometry,  #
        output_size: torch.Size,  #
        get_ray_intersection_fractions: bool = False  #
) -> torch.Tensor:
    device = volume_data.device
    assert len(output_size) == 2
    ...
    img_width: int = output_size[1]
    img_height: int = output_size[0]
    source_position: torch.Tensor = torch.tensor([0., 0., scene_geometry.source_distance], **tensor_kwargs)
    detector_xs: torch.Tensor = detector_spacing[1] * (
            torch.arange(0, img_width, 1, **tensor_kwargs) - 0.5 * float(img_width - 1))
    detector_ys: torch.Tensor = detector_spacing[0] * (
            torch.arange(0, img_height, 1, **tensor_kwargs) - 0.5 * float(img_height - 1))
    ...
    volume_diag: torch.Tensor = torch.tensor(volume_data.size(), **tensor_kwargs).flip(dims=(0,)) * voxel_spacing
    ...
    starts_homogeneous = torch.cat((starts, torch.ones_like(starts[..., 0]).unsqueeze(-1)), dim=-1)
    starts = torch.einsum('ji,...i->...j', h_matrix_inv, starts_homogeneous)[..., 0:3]

    directions_homogeneous = torch.cat((directions, torch.zeros_like(directions[..., 0]).unsqueeze(-1)), dim=-1)
    ...
    ret: torch.Tensor = torch.zeros(output_size, dtype=torch.float32, device=device)
    ...
    if get_ray_intersection_fractions:
        ...
        cuboid_in_centre = torch.zeros(3, **tensor_kwargs)
        ...
        cuboid_above_centre = torch.tensor([0.0, 0.0, z_sum], **tensor_kwargs)
        cuboid_below_centre = torch.tensor([0.0, 0.0, -z_sum], **tensor_kwargs)
        ray_point = torch.einsum(  #
            'ji,i->j',  #
            h_matrix_inv,  #
            torch.cat((source_position, torch.tensor([1.0], **tensor_kwargs)), dim=-1)  #
        )[0:3]
...
@jaxtyped(typechecker=typechecker)
def project_vectors(  #
        vectors: Float64[torch.Tensor, "... 3"],  #
        *,  #
        source_distance: float,  #
        transformation: Transformation  #
) -> Float64[torch.Tensor, "... 2"]:
    """
    :param vectors: (..., 3) tensor
    ...
    ret_homogeneous = torch.einsum("ji,...i->...j", ph_matrix, vectors_homogeneous)
    return ret_homogeneous[..., 0:2] / ret_homogeneous[..., 3].unsqueeze(-1)  # just the x and y components needed


@jaxtyped(typechecker=typechecker)
def get_crop_nonzero_drr(  #
        *,  #
    ...
    return Cropping(right=right.item(), top=top.item(), left=left.item(), bottom=bottom.item())


@jaxtyped(typechecker=typechecker)
def get_crop_full_depth_drr(  #
        *,  #
        image_2d_full: Float32[torch.Tensor, "n m"],  #
        source_distance: float,  #
        current_transformation: Transformation,  #
    ...
    return Cropping(right=1.0, top=top.item(), left=0.0, bottom=bottom.item())
