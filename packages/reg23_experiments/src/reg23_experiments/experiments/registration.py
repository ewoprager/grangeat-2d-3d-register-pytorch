...
def run_reg(  #
    ...
    if dry_run:
        test = obj_fun(torch.rand(2, 6, dtype=torch.float64, device=device))
        assert test.size() == torch.Size([2])
    else:
        # -----
    ...
    for it in range(1, config.iteration_count):
        if dry_run:
            best_so_far = torch.rand(6, dtype=torch.float32, device=device)
        else:
    ...
    return None if dry_run else ret
