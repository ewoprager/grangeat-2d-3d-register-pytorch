...
class XTekCTVolumeLoader(VolumeLoader):
    ...
    def load(self, path: pathlib.Path, series: int) -> Volume | Error:
        ...
        if "voxelsizex" not in config[XTekCTVolumeLoader.CONFIG_SECTION]:
            return Error(
                f"Expected key 'voxelsizex' in section '{XTekCTVolumeLoader.CONFIG_SECTION}' in config file '{config_paths[0]}', but not found.")
        if "voxelsizey" not in config[XTekCTVolumeLoader.CONFIG_SECTION]:
            return Error(
                f"Expected key 'voxelsizey' in section '{XTekCTVolumeLoader.CONFIG_SECTION}' in config file '{config_paths[0]}', but not found.")
        if "voxelsizez" not in config[XTekCTVolumeLoader.CONFIG_SECTION]:
            return Error(
                f"Expected key 'voxelsizez' in section '{XTekCTVolumeLoader.CONFIG_SECTION}' in config file '{config_paths[0]}', but not found.")
        if "voxelsx" not in config[XTekCTVolumeLoader.CONFIG_SECTION]:
            return Error(
                f"Expected key 'voxelsx' in section '{XTekCTVolumeLoader.CONFIG_SECTION}' in config file '{config_paths[0]}', but not found.")
        if "voxelsy" not in config[XTekCTVolumeLoader.CONFIG_SECTION]:
            return Error(
                f"Expected key 'voxelsy' in section '{XTekCTVolumeLoader.CONFIG_SECTION}' in config file '{config_paths[0]}', but not found.")
        if "voxelsz" not in config[XTekCTVolumeLoader.CONFIG_SECTION]:
            return Error(
                f"Expected key 'voxelsz' in section '{XTekCTVolumeLoader.CONFIG_SECTION}' in config file '{config_paths[0]}', but not found.")
        volume = torch.empty((  #
