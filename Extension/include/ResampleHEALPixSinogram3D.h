 ...
 */
at::Tensor ResampleHEALPixSinogram3D_CPU(const at::Tensor &sinogram3d, const at::Tensor &sinogramSpacing,
                                         const at::Tensor &sinogramRangeCentres, const at::Tensor &projectionMatrix,
                                         const at::Tensor &phiValues, const at::Tensor &rValues);

/**
 * @ingroup pytorch_functions
 * @brief An implementation of reg23::ResampleHEALPixSinogram3D_CPU that uses CUDA parallelisation.
 */
__host__ at::Tensor ResampleHEALPixSinogram3D_CUDA(const at::Tensor &sinogram3d, const at::Tensor &sinogramSpacing,
                                                   const at::Tensor &sinogramRangeCentres,
 ...
 */
template <typename texture_t> struct ResampleHEALPixSinogram3D {
	...
	__host__ static CommonData Common(const at::Tensor &sinogram3d, double rSpacing, double rRangeCentre,
	                                  const at::Tensor &projectionMatrix, const at::Tensor &phiValues,
	                                  const at::Tensor &rValues, at::DeviceType device) {
		// sinogram3d should be a 3D tensor of floats on the chosen device, with the correct ratio between the sizes of
		// the last two dimensions
		TORCH_CHECK(sinogram3d.sizes().size() == 3)
		TORCH_CHECK(sinogram3d.sizes()[1] % 4 == 0)
		TORCH_CHECK(sinogram3d.sizes()[2] % 3 == 0)
		TORCH_CHECK(sinogram3d.sizes()[1] / 4 == sinogram3d.sizes()[2] / 3)
		...
		ret.inputTexture = texture_t{sinogramPtr, sinogram3d.sizes()[1] / 4, sinogram3d.sizes()[0], rSpacing,
		                             rRangeCentre, TextureAddressMode::ZERO};
