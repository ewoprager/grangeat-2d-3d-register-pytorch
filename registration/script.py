...
def get_volume_and_sinogram(ct_volume_path: str | None, cache_directory: str, *, load_cached: bool, save_to_cache: bool,
    ...
    if sinogram3d is None:
        res = pre_computed.calculate_volume_sinogram(cache_directory, vol_data, voxel_spacing=voxel_spacing,
            ct_volume_path=ct_volume_path, volume_downsample_factor=volume_downsample_factor,
            save_to_cache=save_to_cache, sinogram_size=sinogram_size, sinogram_type=sinogram_type)
        if res is None:
            return None
        sinogram3d, _ = res
