...
def calculate_volume_sinogram(cache_directory: str, volume_data: torch.Tensor, voxel_spacing: torch.Tensor,
    ...
    sinogram_data = grangeat.calculate_radon_volume(volume_data, voxel_spacing=voxel_spacing,
                                                    output_grid=sinogram3d_grid, samples_per_direction=vol_counts)
    ...
    # vol_image = ScalarImage(tensor=vol_data[None, :, :, :])  # vol_subject = read(vol_image, spacing=voxel_spacing)
    ...
    # sdd=source_distance,  # Source-to-detector distance (i.e., focal length)  #                     height=int(
    # torch.ceil(  #                         1.1 * voxel_spacing.mean() * torch.tensor(vol_size).max() /
    # detector_spacing).item()),  #                     # Image height (if width is not provided, the generated DRR
    # is square)  #                     delx=detector_spacing,  # Pixel spacing (in mm)  #                     ).to(
    # device)  #
