        ...
        if x_ray is None:
            ...
            if drr_spec is None:
                drr_spec = drr.generate_drr_as_target(
                    cache_directory, path, self.ct_volume, self.ct_spacing, device=self.device,
                    save_to_cache=save_to_cache, size=new_drr_size)

            (
                self._fixed_image_spacing, self._scene_geometry, self._image_2d_full, self._sinogram2d,
                sinogram2d_range, self._transformation_ground_truth) = drr_spec
            del drr_spec

            self._fixed_image = self._image_2d_full
            self._sinogram2d_grid = Sinogram2dGrid.linear_from_range(
                sinogram2d_range, self.sinogram2d.size(), device=self.device)
        else:
            # Load the given X-ray
            self._image_2d_full, self._fixed_image_spacing, self._scene_geometry = data.read_dicom(
                x_ray, downsample_factor=4)
            # Flip horizontally
